...
scored as (
  select
    ...
    -- MARKET:
    -- Compare TD probability across the full slate,
    -- because every player is priced on the same outcome:
    -- scoring an anytime touchdown.
    percent_rank() over (
    ...
    case
      when has_usage_history
       and opportunity_role_index is not null
    ...
    case
      when has_usage_history
       and snap_role_index is not null
      then percent_rank() over (
        partition by position
